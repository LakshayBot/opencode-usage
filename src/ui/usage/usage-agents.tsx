/**
 * "By agent" view for the /usage popup.
 *
 ...
 * no agent detail drill-down (rows carry no onSelect). Like the graph subview
 * it registers backspace as "back to the overview" on a keymap layer that
 * strictly outranks the overview's own layer (dialog.replace swaps content but
 * leaves the previous view's bindings registered; equal priority resolves to
 * registration order — older wins).
 */
...
import type { UsageAgentRowModel } from "./usage-view-model.ts"

export function UsageAgentsDialog(props: {
  api: TuiPluginApi
  rows: UsageAgentRowModel[]
  onBack: () => void
}): JSX.Element {
  useBindings(() => ({
