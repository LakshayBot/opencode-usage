...
**Conclusion for `/usage`:** register the TUI command only. The palette merges keymap slash commands AND server commands without dedup (see §5, autocomplete.tsx), and a command file forces a model round-trip that renders the report as a chat message instead of a popup — so the TUI plugin's `slashName: "usage"` opening a native dialog popup is the single registration (zero tokens, works in every model). The `usage` tool stays registered server-side so a model can still render a report when the user types `/usage` directly.
