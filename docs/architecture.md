...
5. **Global command**: write `<config>/commands/usage.md` (frontmatter `description` + template calling the `usage` tool with `$ARGUMENTS`). Auto-discovered; works in TUI (typed-Enter path) and any command-executing client.
